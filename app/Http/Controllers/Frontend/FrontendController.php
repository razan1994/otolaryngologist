...
class FrontendController extends Controller
{
    public function welcome()
    {
        ...
        $treatments=Treatment::latest()->limit(3)->get();
    ...
    public function Blogs()
    {
        ...
        $blogs=Blogs::where('status',1)->paginate(9);

        $seo_operation = SeoOperation::where('page_name', 'Blogs')->get()->first();
        return view('Front.blogs',compact('blogs','seo_operation'));
    }
    public function BlogDetails(Route $route,$aliasname)
    {
        if (Config::get('app.locale') == 'en'){
            $news_blog = Blogs::where('status', 1)->where('alias_name_en','=',$aliasname)->get()->first();
        }else{
            $news_blog = Blogs::where('status', 1)->where('alias_name_ar','=',$aliasname)->get()->first();
        }

        if($news_blog){
            $popular_blogs=Blogs::where('status', 1)->where('id','!=',$news_blog->id)->limit(6)->get();


            return view('Front/blog-details',compact('news_blog','popular_blogs'));
        }else{
            return redirect()->back()->with('danger','Blog Not Found !!!');
        }
    ...
    {
        if (Config::get('app.locale') == 'en'){
            $news_blog = Photo::where('status', 1)->where('alias_name_en','=',$aliasname)->get()->first();
        }else{
            $news_blog = Photo::where('status', 1)->where('alias_name_ar','=',$aliasname)->get()->first();
        }

        if($news_blog){
            $popular_blogs=Photo::where('status', 1)->where('id','!=',$news_blog->id)->limit(6)->get();
            // return  $popular_blogs;

            return view('Front/gallary-details',compact('news_blog','popular_blogs'));
        }else{
    ...
    public function Treatments()
    {
        ...
        $seo_operation = SeoOperation::where('page_name', 'Treatments')->get()->first();
    ...
    {
        if (Config::get('app.locale') == 'en'){
            $treats = Treatment::where('status', 1)->where('alias_name_en','=',$aliasname)->get()->first();
        }else{
            $treats = Treatment::where('status', 1)->where('alias_name_ar','=',$aliasname)->get()->first();
        }

        if($treats){
            $popular_treatments=Treatment::where('status', 1)->where('id','!=',$treats->id)->limit(6)->get();


            return view('Front/treatments-details',compact('treats','popular_treatments'));
        }
        else{
            return redirect()->back()->with('danger','Treatments Not Found !!!');
        }
    }
